...
const en: NotificationLocale = {
  email: {
    footer: 'Du har fått detta eftersom du har aktiverat aviseringar i Mancheng.',
    manage: 'Hantera egenskaper under Inställningar',
    madeWith: 'Gjorde med',
    openTrek: 'Öppna Mancheng',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} bjöd in dig att slå samman semesterplaner. Öppna Mancheng för att acceptera eller avvisa.`,
    }),
    collection_invite: (p) => ({
      title: 'Inbjudan till samling',
      body: `${p.actor} bjöd in dig att dela en samling. Öppna Mancheng för att acceptera eller avvisa.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Ny Mancheng version tillgänglig',
      body: `Mancheng ${p.version} är nu tillgänglig. Gå till adminpanelen för att uppdatera.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hej',
    body: 'Vi har fått en begäran om att återställa lösenordet till ditt Mancheng konto. Klicka på knappen nedan för att ange ett nytt lösenord.',
