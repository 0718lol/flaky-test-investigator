...
const it: NotificationLocale = {
  email: {
    footer: 'Hai ricevuto questa email perché hai le notifiche abilitate in Mancheng.',
    manage: 'Gestisci le preferenze nelle impostazioni',
    madeWith: 'Made with',
    openTrek: 'Apri Mancheng',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} ti ha invitato a fondere i piani vacanza. Apri Mancheng per accettare o rifiutare.`,
    }),
    collection_invite: (p) => ({
      title: 'Invito a una raccolta',
      body: `${p.actor} ti ha invitato a condividere una raccolta. Apri Mancheng per accettare o rifiutare.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nuova versione Mancheng disponibile',
      body: `Mancheng ${p.version} è ora disponibile. Visita il pannello di amministrazione per aggiornare.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Ciao',
    body: 'Abbiamo ricevuto una richiesta di reimpostazione della password per il tuo account Mancheng. Clicca il pulsante qui sotto per impostare una nuova password.',
