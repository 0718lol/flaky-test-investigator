...
const ar: NotificationLocale = {
  email: {
    footer: 'تلقيت هذا لأنك قمت بتفعيل الإشعارات في Mancheng.',
    manage: 'إدارة التفضيلات',
    madeWith: 'Made with',
    openTrek: 'فتح Mancheng',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} يدعوك لدمج خطط الإجازة. افتح Mancheng للقبول أو الرفض.`,
    }),
    collection_invite: (p) => ({
      title: 'دعوة إلى مجموعة',
      body: `${p.actor} يدعوك لمشاركة مجموعة. افتح Mancheng للقبول أو الرفض.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'إصدار Mancheng جديد متاح',
      body: `Mancheng ${p.version} متاح الآن. تفضل بزيارة لوحة الإدارة للتحديث.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'مرحبا',
    body: 'تلقينا طلبًا لإعادة تعيين كلمة المرور لحسابك في Mancheng. انقر على الزر أدناه لتعيين كلمة مرور جديدة.',
