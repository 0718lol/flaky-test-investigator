...
const fr: NotificationLocale = {
  email: {
    footer: 'Vous recevez cet e-mail car les notifications sont activées dans Mancheng.',
    manage: 'Gérer les préférences',
    madeWith: 'Made with',
    openTrek: 'Ouvrir Mancheng',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} vous invite à fusionner les plans de vacances. Ouvrez Mancheng pour accepter ou refuser.`,
    }),
    collection_invite: (p) => ({
      title: 'Invitation à une collection',
      body: `${p.actor} vous invite à partager une collection. Ouvrez Mancheng pour accepter ou refuser.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nouvelle version Mancheng disponible',
      body: `Mancheng ${p.version} est maintenant disponible. Rendez-vous dans le panneau d'administration pour mettre à jour.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Bonjour',
    body: 'Nous avons reçu une demande de réinitialisation du mot de passe de votre compte Mancheng. Cliquez sur le bouton ci-dessous pour définir un nouveau mot de passe.',
