...
const ca: NotificationLocale = {
  email: {
    footer: 'Has rebut això perquè tens les notificacions activades a Mancheng.',
    manage: 'Gestiona les preferències',
    madeWith: 'Fet amb',
    openTrek: 'Obre Mancheng',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} et va convidar a fusionar plans de vacances. Obre Mancheng per acceptar o rebutjar.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Versió nova de Mancheng disponible',
      body: `Mancheng ${p.version} ja està disponible. Visita el panell d'administració per actualitzar.`,
    }),
    ...
    collection_invite: (p) => ({
      ...
      body: `${p.actor} t'ha convidat a compartir una col·lecció. Obre Mancheng per acceptar-la o rebutjar-la.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hola',
    body: 'Vam rebre una sol·licitud per restablir la contrasenya del teu compte de Mancheng. Fes clic al botó de sota per establir una contrasenya nova.',
