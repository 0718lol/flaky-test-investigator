...
You are connected to Mancheng, a travel planning application. Below is a compact reference of the data model, key workflows, and behavioral rules you must follow.
...
The following features are optional and may not be available on every Mancheng instance. Check tool availability before assuming they exist:
...
const STATIC_TOKEN_DEPRECATION_NOTICE =
    ...
    '"Your Mancheng integration is using a static API token that is deprecated and will stop working in a future version. ' +
    'Please migrate to OAuth 2.1: go to Settings → Integrations → MCP → OAuth Clients in Mancheng and register an OAuth 2.1 application." ' +
...
function setAuthChallenge(res: Response, error = 'invalid_token'): void {
  ...
  res.set('WWW-Authenticate',
      `Bearer realm="Mancheng MCP", resource_metadata="${base}/.well-known/oauth-protected-resource/mcp", error="${error}"`);
}
