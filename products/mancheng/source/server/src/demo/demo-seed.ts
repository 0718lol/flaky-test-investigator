...
function seedDemoData(db: Database.Database): { adminId: number; demoId: number } {
  ...
  // Create admin user if not exists. Prefer the canonical username/email pair,
  // but also reuse an existing local admin so preview seeding can run on top of
  // a real install without colliding on UNIQUE(username).
  let admin = db.prepare('SELECT id FROM users WHERE email = ? OR username = ? LIMIT 1').get(ADMIN_EMAIL, ADMIN_USER) as { id: number } | undefined;
  if (!admin) {
  ...
  let demo = db.prepare('SELECT id FROM users WHERE email = ? OR username = ? LIMIT 1').get(DEMO_EMAIL, 'demo') as { id: number } | undefined;
  if (!demo) {
