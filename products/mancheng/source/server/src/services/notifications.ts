    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width: 480px; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.06);">
        ...
        <tr><td style="background: linear-gradient(135deg, #000000 0%, #1a1a2e 100%); padding: 32px 32px 28px; text-align: center;">
          ...
          <div style="color: #ffffff; font-size: 24px; font-weight: 700; letter-spacing: -0.5px;">Mancheng</div>
...
export async function sendPasswordResetEmail(
  ...
  try {
    ...
    await transporter.sendMail({
      from: smtpCfg.from,
      to,
      subject: `Mancheng — ${strings.subject}`,
...
export function buildWebhookBody(
      ...
      embeds: [
        {
          ...
          url: payload.link,
          color: 0x3b82f6,
          footer: { text: payload.tripName ? `Trip: ${payload.tripName}` : 'Mancheng' },
  ...
  if (isSlack) {
    ...
    const link = payload.link ? `\n<${payload.link}|Open in Mancheng>` : '';
    return JSON.stringify({
      text: `*${payload.title}*\n${payload.body}${trip}${link}`,
    });
  }

  return JSON.stringify({ ...payload, timestamp: new Date().toISOString(), source: 'Mancheng' });
}
...
export async function testSmtp(to: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    await transporter.sendMail({
      from: config.from,
      to,
      subject: 'Mancheng — Test Notification',
      text: 'This is a test email from Mancheng. If you received this, your SMTP configuration is working correctly.',
    });
    return { success: true };
  } catch (err) {
...
export async function testWebhook(url: string): Promise<{ success: boolean; error?: string }> {
  try {
    const sent = await sendWebhook(url, {
      ...
      body: 'This is a test webhook from Mancheng. If you received this, your webhook configuration is working correctly.',
    });
...
export async function testNtfy(cfg: {
  ...
  try {
    const sent = await sendNtfy(url, cfg.token ?? null, {
      event: 'test',
      title: 'Test Notification',
      body: 'This is a test notification from Mancheng. If you received this, your ntfy configuration is working correctly.',
    });
