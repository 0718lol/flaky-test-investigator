...
export function previewLogin(): { error?: string; status?: number; token?: string; user?: Record<string, unknown> } {
  try {
    ...
    return { error: err instanceof Error ? err.message : 'Preview login failed', status: 500 };
  }
}
...
export function setupMfa(userId: number, userEmail: string): { error?: string; status?: number; secret?: string; otpauth_url?: string; qrPromise?: Promise<string> } {
  ...
  try {
    ...
    otpauth_url = authenticator.keyuri(userEmail, 'Mancheng', secret);
  } catch (err) {
